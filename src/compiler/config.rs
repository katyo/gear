...
impl Config {
    pub fn backed_by(&mut self, back: &Config) {
        if self.name.is_none() && back.name.is_some() {
            self.name = back.name.clone();
        }
        if self.path.is_none() && back.path.is_some() {
            self.path = back.path.clone();
        }
        if self.triple.is_none() && back.triple.is_some() {
            self.triple = back.triple.clone();
        }
        if let Some(back_flags) = &back.flags {
            if let Some(flags) = self.flags.take() {
                //flags = back.flags.into_iter() + flags.into_iter();
                self.flags = Some(
                    back_flags
                        .iter()
                        .cloned()
